    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :skuImageList="skuImageList" :skuInfo="skuInfo"></Zoom>
          <!-- 小图列表 -->
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <!-- 以前的路由跳转是由A路由跳转到B路由
                但是这里的跳转，在加入购物车进行路由跳转之前，还需要发请求，
                要把用户购买的信息通过请求的形式通知服务器，服务器进行响应的存储等操作
              -->
              <div class="add" @click="shopCartHandler(skuInfo.id, skuNum)">
...
<script lang="ts" setup>
  ...
  let { categoryView, skuInfo, spuSaleAttrList } = storeToRefs(detailStore)
  ...
  const changeSkuNum = () => {
    ...
    if (isNaN(skuNum.value * 1) || skuNum.value < 1) {
      ...
      // 正常大于1【大于1整数不能出现小数，向上取整】
      skuNum.value = ceil(skuNum.value)
    }
  }
  ...
  // 产品的售卖属性值切换高亮 (排他思想)
  const changeActive = (value: detail_type.SpuSaleAttrValueList, list: detail_type.SpuSaleAttrValueList[]) => {
    list.forEach((item) => {
      item.isChecked = '0'
    })
    value.isChecked = '1'
  ...
  // 处理购物车的回调函数
  const shopCartHandler = async (skuId: number, skuNum: number) => {
    // 1：发请求---将产品加入到数据库（通知服务器）
    // 2：服务器存储成功---进行路跳转 并 传递参数
    // 3：失败，给用户提示
    try {
      await detailStore.addOrUpdateShopCart(skuId, skuNum)
      router.push({
        name: 'addcartsuccess',
        query: {
          skuNum,
      ...
      // 产品信息的数据【比较复杂：skuinfo】，通过会话存储（不持久化，会话结束，数据就消失）
      // 本地存储|会话存储，一般存储的是字符串
  ...
  watchEffect(async () => {
    if (route.name == 'detail') {
      await detailStore.getGoodsInfo(Number(route.params.skuid))
    }
  })
</script>
