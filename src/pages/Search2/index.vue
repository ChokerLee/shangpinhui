...
<script lang="ts" setup>
  ...
  // 搜索类*主类
  const searchClass = useSearch(searchParams)
  // 面包屑*副类
  const breadClass = useBread(searchParams, is_go)
  // 排序类*副类
  const paixuClass = usePaixu(searchParams)
  // 品牌类*副类
  const brandClass = useBrand(searchParams)

  /**------------ 搜索类*useSearch ------------*/
  // 分页请求
  const pageSearch = (pagenum: number = 1) => searchClass.pageSearch(pagenum)

  /**------------ 面包屑类*useBread ------------*/
  // 移除分类的面包屑
  const removeCategoryName = () => breadClass.removeCategoryName()
  // 删除关键字
  const removeKeyWord = () => breadClass.removeKeyWord()
  // 删除品牌关键字
  const removeTradeMark = () => breadClass.removeTradeMark()
  // 删除属性关键字
  const removeAttr = (index: number) => breadClass.removeAttr(index)

  /**------------ 排序类*usePaixu ------------*/
  // 综合排序
  const zhOrder = paixuClass.zhOrder
  // 价格排序
  const priceOrder = paixuClass.priceOrder
  // 控制排序箭头显示
  const arrow = paixuClass.arrow
  // 改变商品排序
  const changeOrderSort = paixuClass.changeOrderSort

  /**------------ 品牌类*useBrand*父子组件通信（self父&&searchSelector.vue子） ------------*/
  // 获取品牌信息
  const trademarkInfo = (trademark: searchList_type.TrademarkList) => {
    brandClass.trademarkInfo(trademark)
  }
  // 获取品牌属性信息
  const attrInfo = (attr: searchList_type.AttrsList, attrValue: string) => {
    brandClass.attrInfo(attr, attrValue)
  }

  //* 搜索List,请求数据方法（最重要的一环！！！）
  ...
  watchEffect(async () => {
    // 此判断可以防止路由跳转离开search的时候再次请求，因为goSearchEffect依赖了路由
